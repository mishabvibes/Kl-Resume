...
function EditorContent() {
  ...
  return (
    <main className="min-h-screen p-4 md:p-8 flex flex-col items-center bg-[url('/grid.svg')]">
      ...
      {/* SUCCESS MODAL */}
      {showSuccessModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 animate-in fade-in duration-300 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-white/10 w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="text-center">
              <div className="w-20 h-20 bg-green-500/20 text-green-500 rounded-full flex items-center justify-center mb-6 mx-auto">
                <PartyPopper className="w-10 h-10 animate-bounce" />
              </div>
              <h2 className="text-3xl font-black italic uppercase tracking-tighter text-white mb-2 leading-none">Your portfolio <br /> is live!</h2>
              <p className="text-zinc-500 text-xs font-bold uppercase tracking-widest mb-8">Generated by KL Resume Engine</p>

              <div className="bg-black/40 border border-white/5 rounded-2xl p-4 mb-8">
                <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mb-2 text-left ml-1">Your Personal URL</p>
                <div className="flex items-center justify-between gap-4">
                  <span className="text-purple-400 font-mono text-sm truncate">klresume.in/{formData.username}</span>
                  <button 
                    onClick={() => {
                      navigator.clipboard.writeText(`https://klresume.in/${formData.username}`);
                      alert("URL Copied!");
                    }}
                    className="p-2.5 bg-zinc-800 hover:bg-zinc-700 text-white rounded-xl transition active:scale-90"
                  >
              ...
              <div className="flex flex-col gap-3">
                 <Link 
                   ...
                   target="_blank"
                   className="bg-white text-black font-black py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-zinc-200 transition active:scale-95 uppercase text-sm"
                 >
                   View Portfolio <ExternalLink className="w-4 h-4" />
                 </Link>
                 <button 
                   onClick={() => setShowSuccessModal(false)}
                   className="text-zinc-500 font-black py-4 rounded-2xl hover:bg-white/5 transition uppercase text-[10px] tracking-widest"
                 >
                   Close Editor
                 </button>
              </div>
